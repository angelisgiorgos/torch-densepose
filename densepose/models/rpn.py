...
from densepose.models.anchor_generator import MyAnchorGenerator  # noqa: 401
import torch


class CustomRegionProposalNetwork(nn.Module):
    def __init__(
            self, featmap_names: List[str], anchor_generator: MyAnchorGenerator,
    ...
    def forward(self,
        ...
        return self.rpn(images, image_sizes, filtered_features, targets)
...
def concat_box_prediction_layers(box_cls: List[Tensor], box_regression: List[Tensor]) -> Tuple[
    Tensor, Tensor]:
    box_cls_flattened = []
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(
            self,
            anchor_generator: MyAnchorGenerator,
            head: nn.Module,
        ...
        self.fg_bg_sampler = det_utils.BalancedPositiveNegativeSampler(batch_size_per_image,
                                                                       positive_fraction)
        # used during testing
    ...
    def filter_proposals(
        ...
        levels = [
            torch.full((n,), idx, dtype=torch.int64, device=device) for idx, n in
            enumerate(num_anchors_per_level)
            ]
    ...
    def compute_loss(
            self, objectness: Tensor, pred_bbox_deltas: Tensor, labels: List[Tensor],
        ...
        objectness_loss = F.binary_cross_entropy_with_logits(objectness[sampled_inds],
                                                             labels[sampled_inds])
    ...
    def forward(
        ...
        boxes, scores = self.filter_proposals(proposals, objectness, image_sizes,
