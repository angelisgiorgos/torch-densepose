...
def _resize_image_and_masks(
    ...
    if "masks" in target:
        ...
        mask = torch.nn.functional.interpolate(
            mask[:, None].float(), size=size, scale_factor=scale_factor,
            recompute_scale_factor=recompute_scale_factor
            )[:, 0].byte()
        target["masks"] = mask
    return image, target
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def _onnx_batch_images(self, images: List[Tensor], size_divisible: int = 32) -> Tensor:
        ...
        for i in range(images[0].dim()):
            max_size_i = torch.max(
                torch.stack([img.shape[i] for img in images]).to(torch.float32)).to(torch.int64)
    ...
    def forward(
        ...
        # image_list = MyImageList(images, image_sizes_list)
